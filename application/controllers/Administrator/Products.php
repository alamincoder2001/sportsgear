...
class Products extends CI_Controller {
    ...
    public function getProductStock(){
        ...
        $stock = $this->mt->productStock($inputs->productId, $inputs->size_id, $inputs->color_id);
        echo $stock;
    }
        ...
        $stock = $this->db->query("
            select
                ...
                (select ifnull(sum(prd.PurchaseReturnDetails_ReturnQuantity), 0) 
                    ...
                    and prd.size_id = s.Size_SlNo
                    and prd.color_id = c.color_SiNo
                ...
                (select ifnull(sum(srd.SaleReturnDetails_ReturnQuantity), 0)
                    ...
                    and srd.size_id = s.Size_SlNo
                    and srd.color_id = c.color_SiNo
                ...
                (select ifnull(sum(dmd.DamageDetails_DamageQuantity), 0) 
                    ...
                    and dmd.size_id = s.Size_SlNo
                    and dmd.color_id = c.color_SiNo
                    and dmd.status = 'a'
                ...
                (select ifnull(sum(trd.quantity), 0)
                    ...
                    and trd.size_id = s.Size_SlNo
                    and trd.color_id = c.color_SiNo
            ...
            left join tbl_size s on s.Size_SlNo = pd2.size_id 
            left join tbl_color c on c.color_SiNo = pd2.color_id 
            left join tbl_unit u on u.Unit_SlNo = p.Unit_ID
            where p.status = 'a' and p.is_service = 'false' $clauses
            GROUP BY pd2.Product_IDNo, pd2.size_id, pd2.color_id
        ")->result();
