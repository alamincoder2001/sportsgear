...
class Model_Table extends CI_Model{
    ...
    public function currentStock($clauses = '')
    {
        $stock = $this->db->query("
            select * from(
                ...
                left join tbl_size s on s.Size_SlNo = ci.size_id
                left join tbl_color c on c.color_SiNo = ci.color_id
    ...
    public function productStock($productId, $size_id, $color_id) {
        $stockQuery = $this->db->query("select * from tbl_currentinventory where product_id = ? and size_id = ? and color_id = ? and branch_id = ?", [$productId, $size_id, $color_id, $this->session->userdata("BRANCHid")]);
